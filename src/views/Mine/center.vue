    ...
    <div>
        ...
        <div>
            <input type="file" name="file" value="上传头像" @change="handleToUpload">
             <img v-if= "change" class="userHead" :src="$store.state.user.userHead">
            <img v-if="show" class="userHead" :src="$store.state.user.userHead">
        </div>
    </div>
</template>
...
//        console.log(121)
//        this.url=this.$store.state.user.userHead
//    },
   watch:{
       change(){
           this.show=this.change ? false:true
       }
   },
